      ...
      integer r,error, ti, scanl,midk,midl
      parameter (scanl=200)
      parameter (r=10)
      real*8 fcn,param(mxparm),t,tend,tft,y(neq),Ed,E0,Bdw,x1,x2,dw,dE,dtheta,dthetam
      real*8 vout(2,1000,p*q),mem(2,1000,p*q),forces(8,2000),tuning(2,r)
      real*8 B,y0
      common/func/B,y0,counter

      parameter (dw=.5D5,dE=3D-4,dtheta=1D-3)
      ...
      parameter (pend=1D5,ddw=1D-4,scanprec=5D-2)
      real*8 tftold,hiTstep,lowTstep,wfTstep,bTstep,Vg(p*q),scandata(p,q,0:scanl),scandx(0:scanl*2)
      real*8 timecheck,percent,xf(p,q),yf(p,q),ET0,ET,endpoints(2,p,q)
...
61    format(x,'counter=',i1,9x,'steps=',i10,x,'Tstep=',E12.4)
          ...
          do l=1,q
              do k=1,p
                  ...
                  y(4)=(1.00D0+dE/(4D0*(q-1)/2D0)*(l-midl))*vini*cos(dthetam/2D0*(k-midk)/((p-1)/2D0))
                  y(5)=(1.00D0+dE/(4D0*(q-1)/2D0)*(l-midl))*vini*sin(dthetam/2D0*(k-midk)/((p-1)/2D0))
...
66    format(i3,'%')
      ...
      tuning(1,m)=dthetam
      tuning(2,m)=maxval(xf(1:p,1:q))-minval(xf(1:p,1:q))
