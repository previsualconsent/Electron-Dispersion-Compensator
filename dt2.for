      ...
      call iwkin(43592)
111   format(3A20)
      OPEN(UNIT=11,FILE='dt2.dat',type='replace')
...
61    format(x,'counter=',i1,9x,'steps=',i10,x,'Tstep=',E12.4)
          ...
          do m=1,r
             do n=1,s
          dEm = dE*5D-1+(m-1)*dE
          dthetan = dtheta*5D-1+(n-1+5)*dtheta
          do l=1,q
              do k=1,p
...
66    format(i3,'%')
      ...
      OPEN(UNIT=11,FILE='dt2.dat',status='old',action='WRITE',access='append')
