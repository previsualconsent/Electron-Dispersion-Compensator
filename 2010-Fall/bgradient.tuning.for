...
c      endoftim=endoftim*1.E-6 
	...
	real*8 vout(2,1000,p*q),mem(2,1000,p*q)!,dataout(7,2,p),weintime(7,p)
        ...
        real*8 dw1,dw2,dummy,ddw,wfls,bls,percent
...
61    format(x,'counter=',i1,8x,'steps=',i10)
	...
	do k=1,p
		...
		do i=1,neq
		   y(i)=0d0
		enddo
	y(4)=(1.00D0+2D0**(m-1)*2.5D-5*(l-(q+1)/2))*vini*cos(.001D0*(k-(p+1)/2)) !+vini*(k-(p+1)/2)*.005D0
	y(5)=(1.00D0+2D0**(m-1)*2.5D-5*(l-(q+1)/2))*vini*sin(.001D0*(k-(p+1)/2))	  !(1+0.01*(j-3))
c		y(2)=j*0.001   !(start at 11 position separated by 1 mm each)
c		write(6,*) y(2)
...
c				dataout(4,6,k)=tft
			...
			if (counter.eq.4) then 
                                !sign of wienfilter should be negative
				Ey=Ed
               Bz=Ey/(vini+(e*B/(4D0*me)+e*Ey/(me*vini))*(y(2)-y0))
               !Bz=E0/y(4)
				!Bz=Ey/vini
			endif
			if (counter.eq.5) then 
