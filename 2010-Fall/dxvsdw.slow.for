...
c      endoftim=endoftim*1.E-6 
	...
	parameter (mxparm=120,neq=6,p=3,q=1)
	integer divider,r
        parameter (divider=9000,r=5)
	...
	!real*8 mem(2,1000,p*q),dataout(7,2,p),vout(1000,p,2),weintime(7,p)
        ...
        real*8 dw1,dw2,dummy,ddw
        parameter (pend=1D0,ddw=1D-4)
...
c       !run test electron to get information about path
	...
	lowstep=length/(((pend*ddw/10D0)/4D0)/10D0) !set how small the steps are
                                !based how wide the shortest boundary is
        ...
        E0=-B*vini/4D0          !sets the value of the Wien filter so c=-1
                ...
                lowTstep=endoftim/lowstep       !length of integration time for
                                                !low res
                sls=lowTstep*vini        !distance infront of border to go to hi-res
                histep=1000*endoftim/(sls/vini) !hi-res step for approaching borders 
...
60    format(x,'counter=',i1,x,'flag=',i1,x,'step=',i10)
...
61    format(x,'counter=',i1,8x,'step=',i10)
...
50    format(x,i1,'/',i1,x,i1,'/',i1,x,i2,'/',
