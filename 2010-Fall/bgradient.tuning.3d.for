...
c      endoftim=endoftim*1.E-6 
      ...
      yprime(1)=y(4)
        ...
        common/time/ endoftim
        real*8 endoftim
        integer mxparm,neq,i,ido,p,q,r,s,k,l,z,m,n,time(6),lowstepcheck
        parameter (mxparm=120,neq=6,p=3,q=1)
        integer divider,error
        parameter (divider=260,r=10, s=5)
        real*8 fcn,param(mxparm),t,tend,tft,y(neq),B,y0,Ed,E0,Bdw,x1,x2,dw
        parameter (B=.031D0,dw=.75D-1)
            ...
            do l=1,q
              do k=1,p
                ...
                do i=1,neq
                  y(i)=0d0
                enddo
                y(4)=(1.00D0+2D0**(m)*2.5D-5*(l-(q+1)/2))*vini*cos(.001D0*n*(k-(p+1)/2)) !+vini*(k-(p+1)/2)*.005D0
                y(5)=(1.00D0+2D0**(m)*2.5D-5*(l-(q+1)/2))*vini*sin(.001D0*n*(k-(p+1)/2))   !(1+0.01*(j-3))
...
c                 dataout(4,6,k)=tft
     ...
     1maxval(comp(1:p,1:q))-minval(comp(1:p,1:q))
            if (error.eq.1) then
              ...
              write(12,101) 2D0**(m)*2.5D-5,.001D0*n, 
     1maxval(comp(1:p,1:q))-minval(comp(1:p,1:q))
            else
              write(11,101) 2D0**(m)*2.5D-5,.001D0*n, 
     1maxval(comp(1:p,1:q))-minval(comp(1:p,1:q))
            endif
            lowstepcheck=0
          enddo
