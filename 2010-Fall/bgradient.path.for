...
c      endoftim=endoftim*1.E-6 
	...
	real*8 vout(2,1000,p*q),mem(2,1000,p*q),weintime(7,p*q)!,dataout(7,2,p),
        ...
        real*8 dw1,dw2,dummy,ddw,wfls,bls,disp(1:p*q)
...
61    format(x,'counter=',i1,8x,'steps=',i10)
	...
	y(4)=(1.00D0+2D0**9*2.5D-5*(l-(q+1D0)/2D0))*vini*cos(.001D0*(k-(p+1D0)/2D0))
	y(5)=(1.00D0+2D0**9*2.5D-5*(l-(q+1D0)/2D0))*vini*sin(.001D0*(k-(p+1D0)/2D0))
c		y(2)=j*0.001   !(start at 11 position separated by 1 mm each)
c		write(6,*) y(2)
...
50    format(x/,x/,x,i1,'/',i1,x,i1,'/',i1,x,i2,'/',
     1x,i2/,' Ed=',E12.2,x,'v0=',E12.2//)
		...
		tft=0D0
            ...
            vout(2,test,k+p*(l-1))=y(4)!sqrt(y(4)*y(4)+y(5)*y(5))
			endif
                        i=i+1
			tft=tft+step
			tend=tft  
...
c               	if ((y1old.le.pend-sls).and.(y(1).gt.pend-sls)
c     1.and.(counter.eq.8)) then
        if ((tft.gt.endoftim-lowTstep*2D0).and.
     1(tftold.le.endoftim-lowTstep*2D0)) then
                                step=hiTstep
                                lowstepcheck=lowstepcheck+1
                               !write(6,60) counter, 7, i
                        endif
...
c				dataout(4,6,k)=tft
			...
			if (counter.eq.4) then 
                                !sign of wienfilter should be negative
				Ey=Ed
               Bz=E0/(vini+(e*B/(4D0*me)+e*E0/(me*vini))*(y(2)-y0))
               !Bz=E0/y(4)
				!Bz=Ey/vini
			endif
			if (counter.eq.5) then 
		...
		!dataout(6,2,k)=y(4)-vini
                ...
                vout(2,1000,k+p*(l-1))=y(4)!sqrt(y(4)*y(4)+y(5)*y(5))
